...
///
/// ```
pub fn bridge(cmd_buffer: usize) -> Result<(DhtClient, DhtServer), BridgeError> {
    let swarm = SwarmBuilder::with_new_identity()
        ...
        .map_err(|err| BridgeError::Initialization(err.to_string()))?
        .with_dns()
        .map_err(|err| BridgeError::Initialization(err.to_string()))?
        .with_behaviour(|key| Behaviour {
            ...
            identify: {
                let config = identify::Config::new("/ipfs/id/1.0.0".to_owned(), key.public());
                identify::Behaviour::new(config)
            },
        })
        .map_err(|err| BridgeError::Initialization(err.to_string()))?
        .with_swarm_config(|c| c.with_idle_connection_timeout(Duration::from_secs(60 * 30)))
        .build();
    ...
    Ok((
        DhtClient::new(command_sender, *swarm.local_peer_id()),
...
pub enum BridgeError {
    #[error("failed to initialize the bridge DHT connection: {0}")]
