...
impl DhtClient {
    pub(crate) fn new(sender: Sender<CommandCallback>, peer_id: PeerId) -> Self {
        Self {
    ...
    #[inline(always)]
    pub const fn get_peer_id(&self) -> PeerId {
        self.peer_id
    }

    #[inline(always)]
    async fn send_command(&self, command: Command) -> CommandResult {
        let (callback_sender, receiver) = oneshot::channel();
        let mut sender_lock = self.sender.lock().await;
        sender_lock.send((command, callback_sender)).await?;
        drop(sender_lock);
        receiver.await?
    }
    ...
    pub async fn listen_on(&self, addr: impl Into<Multiaddr>) -> CommandResult {
        self.send_command(Command::AddListener { addr: addr.into() })
            .await
    }
    ...
    pub async fn remove_listener(
        ...
        self.send_command(Command::RemoveListener { listener_id })
            .await
    }

    pub async fn get_all_listeners(&self) -> CommandResult {
        self.send_command(Command::GetAllListeners).await
    }

    pub async fn dial_peer_with_id_addr(
        &self,
        peer_id: impl Into<PeerId>,
        addr: impl Into<Multiaddr>,
    ) -> CommandResult {
        self.send_command(Command::DialWithIdAddr {
